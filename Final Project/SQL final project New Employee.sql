...
select min(salary) from personaldetails group by department;
select max(salary) from personaldetails group by department;

select department, count(*) as total_employee from personaldetails group by department;

select AVG(salary) from personaldetails where department = "Sales";
